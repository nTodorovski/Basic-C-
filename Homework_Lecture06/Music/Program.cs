...
namespace Music
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            FansArray.Where(x => x.FirstName == "Jerry").First().FavoriteSongs = Songs.Where(x => x.Title.StartsWith("B")).ToList();
            FansArray.Where(x => x.FirstName == "Maria").First().FavoriteSongs = Songs.Where(x => x.Length > 360).ToList();
            FansArray.Where(x => x.FirstName == "Jane").First().FavoriteSongs = Songs.Where(x => x.Genre == Genre.Rock).ToList();
            FansArray.Where(x => x.FirstName == "Stefan").First().FavoriteSongs = Songs.Where(x => x.Length < 180).Where(x => x.Genre == Genre.Hip_Hop).ToList();

            List<Person> peopleWithFourOrMoreSongs = FansArray.Where(x => x.FavoriteSongs.Count >= 4).ToList();

            // Gi dava Jerry, Maria, Jane
            Console.WriteLine("People with 4 or more songs:");
            foreach (var item in peopleWithFourOrMoreSongs)
            {
